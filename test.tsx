...
export default function ShopPageClient({
  ...
  // Responsive column count to match Tailwind grid cols
  ...
  // Fetch more products, memoized for stable reference in observer
  ...
  }, [loading, hasMore, page, limit, products, initialTotalCount]);
  ...
  // Reset when initial props change (SSR navigation)
  useEffect(() => {
  ...
  // react-window grid calculation
  const rows = Math.ceil(products.length / columns);
  const CARD_WIDTH = 280; // approximate width including gaps/padding
  const CARD_HEIGHT = 380; // approximate height including gaps/padding
  ...
  const Cell = memo(
    ({
      ...
      return (
        <div style={{ ...style, padding: 12 /* gap-6 approx */ }}>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 pb-24 mt-20">
      ...
      <section className="relative py-16 md:py-24 overflow-hidden">
        {/* Your existing animated backgrounds and overlays */}
      ...
      <section className="container mx-auto px-4 md:px-6 mt-12">
        ...
        {products.length > 0 ? (
          <Grid
            ...
            width={columns * CARD_WIDTH + 24} // +padding approx
            style={{ margin: "0 auto" }}
          >
            {Cell}
          </Grid>
